...
const NOT_FOUND_MSG = (word) => `해당 한국어 단어를 찾을 수 없습니다: ${word}`;
...
function formatEntry(item) {
  let headword = cleanHtml(item.expEntry || '');
  const hanja = cleanHtml(item.expKoreanHanja || '');
  if (hanja) headword = `${headword} (${hanja})`;
  ...
  async handle(payload, ctx) {
    ...
    if (!word) {
      return {
        ...
        meta: { word, source: 'naver.ko', found: false }
      };
    }
    const url = `https://ko.dict.naver.com/api3/koko/search?query=${encodeURIComponent(word)}&range=word`;
    try {
      ...
      return {
        content,
        meta: { word, source: 'naver.ko', found: true, entryCount: Math.min(items.length, MAX_ENTRIES) }
      };
    } catch (err) {
      ctx.log?.('dictionaryKo fetch error', err?.code, err?.message);
      return {
        content: NETWORK_MSG,
